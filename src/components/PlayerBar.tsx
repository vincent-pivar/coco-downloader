...
export function PlayerBar({
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-lg border-t border-slate-200 px-4 py-3 z-50 shadow-2xl"
    >
      <div className="container mx-auto max-w-5xl flex items-center justify-between gap-4">
        {/* Track Info */}
        <div className="flex items-center gap-4 w-1/4 min-w-[200px]">
          <div className={cn(
            "w-12 h-12 rounded-full bg-slate-100 border border-slate-200 overflow-hidden relative flex-shrink-0",
            isPlaying && "animate-spin-slow"
          )} style={{ animationDuration: '8s' }}>
            {currentMusic.cover ? (
              <img src={currentMusic.cover} alt={currentMusic.title} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full flex items-center justify-center bg-sky-100 text-sky-500 font-bold">
                {currentMusic.title[0]}
              </div>
            )}
          </div>
          <div className="flex flex-col overflow-hidden">
            <h3 className="font-semibold text-slate-800 truncate text-sm">{currentMusic.title}</h3>
            <p className="text-xs text-slate-500 truncate">{currentMusic.artist}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center flex-1 max-w-lg">
          ...
          <div className="w-full flex items-center gap-3 text-xs text-slate-400 font-medium">
            ...
            >
              <div 
                className="absolute top-0 left-0 h-full bg-sky-400 rounded-full group-hover:bg-sky-500 transition-colors"
                style={{ width: `${(currentTime / duration) * 100}%` }}
              />
              {/* Thumb */}
              <div 
                className="absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-white border border-slate-300 shadow-sm rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
        ...
        <div className="w-1/4 flex justify-end items-center gap-4 min-w-[150px]">
          <div className="flex items-center gap-2 group w-32">
            ...
            <div 
              className="flex-1 h-1 bg-slate-200 rounded-full cursor-pointer relative overflow-hidden"
              onClick={(e) => {
                const rect = e.currentTarget.getBoundingClientRect();
